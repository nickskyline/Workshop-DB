...
-- SELECT DISTINCT cliente_codigo FROM pagos DATE_FORMAT('fecha_pago', '%Y')
...
SELECT AVG(total_pago) FROM pagos 
...
WHERE 
  e.id NOT IN (SELECT id_empleado FROM cliente WHERE id_empleado IS NOT NULL);
...
FROM cliente c
LEFT JOIN pagos p ON c.id = p.cliente_codigo
GROUP BY c.nombre_cliente;